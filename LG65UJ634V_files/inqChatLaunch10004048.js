/* Timestamp: Thu Apr 12 00:02:15 PDT 2018*//* Checking the supported browsers */
if (!window["v3Lander"] && navigator.userAgent.indexOf("Opera") == -1
	&& navigator.userAgent.indexOf("MSIE 6") == -1 /* We do not support IE7- */
	&& (navigator.userAgent.indexOf("MSIE 7") == -1
		|| navigator.userAgent.indexOf("Trident") != -1
		|| navigator.userAgent.toLowerCase().indexOf("media center") != -1
	)
) {

	if (typeof window.v3LanderConfig != "object") {
		// if custom parameters is not defined or invalid, then set empty map
		window.v3LanderConfig = {};
	}
window.v3Lander = {

	FRAME_ID: "inqChatStage",
	codeVersion:'1523516535842',
	v3Frame:false,
	domState: "defer-failed",
	domReady: false,
	secureProtocol: function(url){
		return url.replace(/^HTTPS?:/i, "https:");
	},
	hostToPath:{},
	hostedFileURL: window.v3LanderConfig.hostedFileURL || "/inqChat.html",
	...
	_getHostedFileURL:function(domain){
		if (!domain) {
			/* if domain has not been supplied, use our domain */
			domain = window.location.host;
		}
		domain = domain.toUpperCase();
		/* If we have no host-to-path map, then return the default URL */
		if (v3Lander.hostToPath && v3Lander.hostToPath[domain]) {
			return v3Lander.hostToPath[domain];
		} else {
			return v3Lander.hostedFileURL;
		}
	},

	_setHostedFileURL: function(domain) {
		if (!domain) {
			/* if domain has not been supplied, use current window domain */
			domain = window.location.host;
		}
		domain = domain.toUpperCase();
		/* If we have no host-to-path map, then hosted file URL is not updated */
		if (v3Lander.hostToPath && v3Lander.hostToPath[domain]) {
			v3Lander.hostedFileURL = v3Lander.hostToPath[domain];
		}
	},
	...
	establishHostedFileURL:function(){
		if (window.v3LanderConfig.hostToPath) {
			for (var i = 0, paths = window.v3LanderConfig.hostToPath; i < paths.length; i++) {
				if (paths[i].domain && paths[i].hostedFileDirectory) {
					v3Lander.hostToPath[paths[i].domain.toUpperCase()] = paths[i].hostedFileDirectory + v3Lander.hostedFileURL;
				}
			}
		}
		this._setHostedFileURL();
	},
	...
	fixV3IFrames:function(div){
		...
		for (var ix = 0, l = inqFrames.length; ix < l; ix++) {
	...
	_reload: function(){
        ...
        window.Inq = undefined;
		this.start();
	},
	createV3Frame: function(win){
		if (top.document.getElementById(v3Lander.FRAME_ID)) {
			return;
		}
        if (!win) win = window;
		...
		var iframesrc = v3Lander.hostedFileURL.indexOf("/") == 0 ?
				doc.location.protocol + '//' + doc.location.hostname + port + v3Lander.hostedFileURL
					+ '?IFRAME' + (window.v3LanderConfig.iframeSrcParams || "")
				: v3Lander.hostedFileURL ;

		if (typeof(CEAPI_POST_CHAT_SURVEY_DATA) != "undefined" && CEAPI_POST_CHAT_SURVEY_DATA != null) {
			// We are loading from tagserver servlet /showPostChatSurvey responded page now. We need to use the
			// same /showPostChatSurvey url to load the inqLaunch<site>.js into the IFrame to
			// avoid cross-domain issue. We also pass in the hostedFileURL param to let /showPostChatSurvey
			// servlet know it is not called from ceapi client. Also passing in the siteID and vanityDomain.
			iframesrc = doc.location + '?IFRAME&hostedFileURL=true&siteID=' + 10004048 +
				'&vanityDomain=' + v3Lander.secureProtocol(window.v3LanderConfig.vanityDomain || 'https://dixons.inq.com');
		}
		...
		if (bdy){
			...
			for (var ix = iframes.length - 1; ix > -1; --ix) {
				bdy.appendChild(iframes[ix]);
			}
			for (var ix = divs.length - 1; ix > -1; --ix) {
				bdy.appendChild(divs[ix]);
			}
		}
		if (iframesrc==this.hostedFileURL) {
			var stg = document.getElementById("inqChatStage");
			var doc = stg.contentDocument || stg.contentWindow.document;
			if (!doc) {
				return;
			}
			doc.open("replace");
			doc.write(
				...
				+'\t\tsrc="' + v3Lander.secureProtocol(window.v3LanderConfig.vanityDomain || 'https://dixons.inq.com')+'/chatskins/launch/inqChatLaunch10004048.js"><\/script>\n'
				+'</body>\n'
				+'</html>');
			doc.close();
		}
    ...
    loadScript:function(name, useMediaServer) {
        var bdy = document.getElementsByTagName("BODY");
        if (bdy && bdy[0]) {
            var srctag = document.createElement("SC" + "RIPT");
			if (useMediaServer) {
				srctag.src = v3Lander.secureProtocol(window.v3LanderConfig.vanityDomain || "http://media-eu1.inq.com" || "https://dixons.inq.com") + name;
			} else {
				srctag.src = v3Lander.secureProtocol(window.v3LanderConfig.vanityDomain || "https://dixons.inq.com") + name;
			}
    ...
    resolvePage: function() {
        var requestUrl = "/tagserver/launch/resolvePage?siteID=10004048&url=" + encodeURIComponent(document.URL) + "&codeVersion=" + encodeURIComponent(v3Lander.codeVersion);
    ...
    applyPage: function(data) {
		...
		if (this.reInitChatData) { //check if Inq#reinitChat callback is required (only for RPL sites)
			window.Inq.reinitChatV3Callback(this.reInitChatData.pageID, this.reInitChatData.data);
			this.reInitChatData = null;
		} else {
			this.createV3Frame();
		}
    },
    ...
    loadTcFramework:function() {
		v3Lander.loadScript(v3Lander.generatePath("tcFramework.min.js"), true);
    },

	generatePath:function(filename) {

		return (window.v3LanderConfig.vanityDomain ? "/chatskins" : "/media") + "/launch/" + filename + "?codeVersion=" + encodeURIComponent(v3Lander.codeVersion);
	},
	...
	loadMbuDataForBU:function(buID, callback){
		...
		v3Lander.loadScript("/chatskins/launch/site_10004048_bu_" + buID + ".js?codeVersion=" + encodeURIComponent(v3Lander.codeVersion));
	},
	...
	insertLandingTag:function(){
		v3Lander.loadScript(v3Lander.generatePath("site_10004048_default.js"), true);
     },
    
    insertPostToServerTag:function(){
		v3Lander.loadScript("/tagserver/postToServer.js?codeVersion=" + encodeURIComponent(this.codeVersion));
    },
	...
	injectIntoParent: function() {
		function whenLoaded() {
			win.v3Lander = window.v3Lander;
			instance.createV3Frame(win);
		}
		var instance = this;
        var win = top.top;
        var doc = win.document;
		...
		} else if (doc.readyState === "complete" || doc.readyState === "loaded") {
        	whenLoaded();
		}
        else {
	...
	testReady:function(){
        var WAIT_PERIOD = 100;
		var skipDomReadyTest = false;

        if (typeof jQuery != "undefined" && jQuery.isReady) {
			v3Lander.setDomReadyTimeout(1);
		} else if (skipDomReadyTest && !v3Lander.v3Frame) {
			v3Lander.setDomReadyTimeout(1);
        } else if (typeof document.readyState == "undefined") {
			if (document.addEventListener){
				document.addEventListener("load",v3Lander.setDomReadyTimeout, false);
				document.addEventListener("DOMContentLoaded", v3Lander.setDomReadyTimeout, false);
			}
			v3Lander.setDomReadyTimeout(WAIT_PERIOD);
		} else {
			...
			if (document.readyState=="complete" && newIeVersion) {
				...
				try {
					...
					if (isExitChat) {
					    ...
					    if (this.isAtHostedFileURL) {
							document.write('<iframe id="tcBBFrame" src="' + v3Lander.secureProtocol(window.v3LanderConfig.vanityDomain || 'https://dixons.inq.com')+'/tagserver/v3/blank.html" ' +
	...
	start:function(){
		try {
			this.assignDomain();
			v3Lander.v3Frame= (window.frameElement && window.frameElement.id == "inqChatStage");
	...
	v3Lander.hostToPath["TC-IMP.INQ.COM"]="/Implementation/mysteryshop/dixons/inqChat.html";
 

	v3Lander.establishHostedFileURL();
	if (window.location.href.indexOf("surveys/launchNativePostChatSurvey") > -1) {
		// In launchNativePostChatSurvey mode, it is at hosted file URL if url contains hostedFileURL=true.
		// See createV3Frame where it inserts hostedFileURL=true into the url.
		(v3Lander).isAtHostedFileURL = window.location.href.indexOf("hostedFileURL=true") > -1;
	} else {
	...
	if (window.location.href.indexOf("?BLNK")!=-1) {}
        ...
        var host = (window.v3LanderConfig.vanityDomain || "https://dixons.inq.com").replace(/^https?:\/\//, ""); // Strip the protocol. Domain only.
		...
		var src = (window.v3LanderConfig.vanityDomain || "https://dixons.inq.com") + "/tagserver/cctoken/getToken" + params;
		s.src = src;
		...
		// Need to pass the parameters along...
		var params = window.location.href.replace(/^.*\?/,"?");
		var src = (window.v3LanderConfig.vanityDomain || "https://dixons.inq.com") + "/tagserver/cctokentest/getToken" + params;
		s.src = src;
		document.head.appendChild(s);
	}
	...
	else if (window.location.href.indexOf("?INJECT")!=-1) {
		v3Lander.injectIntoParent();
	}
	...
	The name field of the window is "RESIZE||iframename||width||height */
		...
		if (!window.v3LanderConfig.customLaunching) {
			v3Lander.start();
		}
	}
}
